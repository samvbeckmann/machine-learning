...
public class Environment {
    ...
    public static void main(String[] args) {
        ...
        for(int i=0; i<rounds; i++) {
            ...
            while(!endState(state)) {
                ...
                //Validate move
                if(state[move]!=0) {
                    System.out.println("Invalid move from player: " + (player==-1 ? "X" : "O") + " " + move + ". Ending simulation.");
                    System.exit(1);
                }
    ...
    public static boolean xWins(int id) { return xWins(getBoard(id));}
    public static boolean oWins(int id) { return oWins(getBoard(id));}
    public static boolean catWins(int id) { return catWins(getBoard(id));}
    public static boolean endState(int id) { return endState(getBoard(id));}
    public static int[] getActions(int id) {return getActions(getBoard(id));}
    public static int[] getNewBoard(int id, int move, int player) {return getNewBoard(getBoard(id), move, player);}
}
